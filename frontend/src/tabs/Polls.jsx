  ...
  return (
    <div>
      ...
      <div className="form-row">
        ...
        <input className="input" style={{maxWidth:120}} value={betAmount} onChange={e=>setBetAmount(e.target.value)} />
      </div>
      <div className="form-row">
      ...
      <div style={{display:"flex", gap:8, marginBottom:12}}>
        <button className="btn" onClick={createPoll} disabled={loading}>{loading ? "Создается..." : "Создать опрос"}</button>
            ...
            {p.options && p.options.map(opt => (
              <div key={opt.id} className="option">
                ...
                <div style={{display:"flex", gap:8}}>
                  ...
                  <button className="btn" onClick={()=>placeBet(p.id, opt.id)}>Сделать ставку</button>
                </div>
              </div>
            ))}
          </div>
